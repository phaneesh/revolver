...
public class RevolverHttpApiSplitTest extends BaseRevolverTest {
    ...
    public void testSimpleSplitHttpCommand() throws TimeoutException {
        ...
        stubFor(get(urlEqualTo("/v2/test")).willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")));
        stubFor(get(urlEqualTo("/v1/split")).willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")));

        RevolverHttpCommand httpCommand = RevolverBundle.getHttpCommand("test", "test_split");
        val request = RevolverHttpRequest.builder().service("test").api("test_split").method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/split").build();
    ...
    public void testFalseSplitHttpCommand() throws TimeoutException {
        ...
        SplitConfig splitConfigv3 = SplitConfig.builder().path("/v3/test").wrr(0.4).build();

        List<SplitConfig> falseSplitConfigs = Lists.newArrayList(splitConfigv1, splitConfigv2, splitConfigv3);
        RevolverConfig.builder().mailBox(InMemoryMailBoxConfig.builder().build()).serviceResolverConfig(ServiceResolverConfig.builder().namespace("test").useCurator(false).zkConnectionString("localhost:2181").build()).clientConfig(ClientConfig.builder().clientName("test-client").build()).global(new RuntimeConfig()).service(RevolverHttpServiceConfig.builder().authEnabled(false).connectionPoolSize(1).secured(false).service("test").type("http").api(RevolverHttpApiConfig.configBuilder().api("test_false_split").method(RevolverHttpApiConfig.RequestMethod.GET).method(RevolverHttpApiConfig.RequestMethod.POST).method(RevolverHttpApiConfig.RequestMethod.DELETE).method(RevolverHttpApiConfig.RequestMethod.PATCH).method(RevolverHttpApiConfig.RequestMethod.PUT).method(RevolverHttpApiConfig.RequestMethod.HEAD).method(RevolverHttpApiConfig.RequestMethod.OPTIONS).path("{version}/test_false_split").splitConfig(RevolverHttpApiSplitConfig.builder().enabled(true).splits(falseSplitConfigs).build()).runtime(HystrixCommandConfig.builder().threadPool(ThreadPoolConfig.builder().concurrency(1).timeout(20000).build()).build()).build())

                .build());
        RevolverHttpCommand httpCommand = RevolverBundle.getHttpCommand("test", "test_false_split");
        val request = RevolverHttpRequest.builder().service("test").api("test_false_split").method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/test_false_split").build();
        httpCommand.execute(request);
    }

    /*@Test
    public void testSingleSplitHttpCommand() throws TimeoutException {
        stubFor(get(urlEqualTo("/v4/test")).willReturn(aResponse().withStatus(200)
    ...
    public void testSimpleSplitServiceHttpCommand() throws TimeoutException {
        stubFor(get(urlEqualTo("/v1/test_service_split")).willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")));

        RevolverHttpCommand httpCommand = RevolverBundle.getHttpCommand("test", "test_service_split");
        val request = RevolverHttpRequest.builder().service("test").api("test_service_split").method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/test_service_split").build();
        val response = httpCommand.execute(request);
        assertEquals(200, response.getStatusCode());
   ...
   /* @Test
    ...
    public void testFalsePathExpressionSplitServiceHttpCommand() throws TimeoutException {
        ...
        RevolverHttpCommand httpCommand = RevolverBundle.getHttpCommand("test", "test_path_expression");
        val request = RevolverHttpRequest.builder().service("test").api("test_path_expression").method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/test_path_expression").build();
        val response = httpCommand.execute(request);
