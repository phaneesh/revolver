...
public class RevolverConfigUpdater implements Runnable {
    ...
    private void updateAvgOfMetrics(Map<String, Integer> keyVsMetricCount, Map<String, OptimizerAggregatedMetrics> optimizerAggregatedMetricsMap, Map<String, Number> aggregatedAppLevelMetricsValues, AtomicLong metricsCount) {
        ...
        keyVsMetricCount.forEach((k, v) -> {
            ...
            optimizerAggregatedMetrics.getMetricsAggValueMap().forEach((metric, aggregatedValue) -> {
                if (v != 0) {
                    optimizerAggregatedMetrics.getMetricsAggValueMap().put(metric, aggregatedValue.intValue() / v);
                }
            });
        });
    ...
    private void updateLatencyThreshold(Map<String, Number> aggregatedAppLevelMetricsValues) {
        ...
        if (optimizerConfig.getTimeConfig() == null || aggregatedAppLevelMetricsValues.get(optimizerConfig.getTimeConfig().getAppLatencyMetric()) == null) {
            return;
        }
        int latencyThresholdValue = aggregatedAppLevelMetricsValues.get(optimizerConfig.getTimeConfig().getAppLatencyMetric()).intValue();
    ...
    private void aggregateAppLevelMetrics(Map<String, Number> aggregatedAppLevelMetricsValues, String metric, Number value, AtomicLong metricsCount) {
        ...
        if (!optimizerConfig.getTimeConfig().getAppLatencyMetric().equals(metric) || value.intValue() == 0) {
            return;
        }
        ...
        if (OptimizerUtils.LATENCY_PERCENTILE_995.equals(metric)) {
            log.error("Aggregated 99%ile for app : " + aggregatedAppLevelMetricsValues.get(metric).intValue() / metricsCount.get());
        }
    }
    ...
    private void aggregateApiLevelMetrics(OptimizerMetrics optimizerMetrics, Map<String, Number> aggregatedMetricsValues, String metric, Number value, Map<String, Integer> keyVsMetricCount, OptimizerCacheKey key) {
        ...
        switch (aggregationAlgo) {
            case MAX:
                if (aggregatedMetricsValues.get(metric) == null || aggregatedMetricsValues.get(metric).intValue() < value.intValue()) {
    ...
    private void updatedApiSettings(RevolverServiceConfig revolverServiceConfig, RevolverHttpApiConfig api, Map<String, OptimizerAggregatedMetrics> optimizerAggregatedMetricsMap, AtomicBoolean configUpdated) {
        ...
        updateConcurrencySetting(api.getRuntime().getThreadPool(), optimizerAggregatedMetrics, configUpdated, api.getApi());
        updateTimeoutSettings(api.getRuntime().getThreadPool(), optimizerAggregatedMetrics, configUpdated, api);
    ...
    private void updateConcurrencySetting(ThreadPoolConfig threadPoolConfig, OptimizerAggregatedMetrics optimizerAggregatedMetrics, AtomicBoolean configUpdated, String poolName) {
        if (optimizerAggregatedMetrics.getMetricsAggValueMap().get(OptimizerUtils.ROLLING_MAX_ACTIVE_THREADS) == null) {
            return;
        }
        OptimizerConcurrencyConfig concurrencyConfig = optimizerConfig.getConcurrencyConfig();
        log.info("Enabled : {}, MaxThreadsMultiplier : {}, MaxThreshold : {}", concurrencyConfig.isEnabled(), concurrencyConfig.getMaxThreadsMultiplier(), concurrencyConfig.getMaxThreshold());
        int maxRollingActiveThreads = optimizerAggregatedMetrics.getMetricsAggValueMap().get(OptimizerUtils.ROLLING_MAX_ACTIVE_THREADS).intValue();
        ...
        if ((maxRollingActiveThreads > concurrency * concurrencyConfig.getMaxThreshold() || maxRollingActiveThreads < concurrency * concurrencyConfig.getMinThreshold()) && maxRollingActiveThreads < threadPoolConfig.getInitialConcurrency() * concurrencyConfig.getMaxThreadsMultiplier()) {
    ...
    private void updateLatencySettings(RevolverHttpApiConfig api, OptimizerAggregatedMetrics optimizerAggregatedMetrics) {
        ...
        int apiLatency = optimizerAggregatedMetrics.getMetricsAggValueMap().get(latencyMetric) == null ? 0 : optimizerAggregatedMetrics.getMetricsAggValueMap().get(latencyMetric).intValue();
