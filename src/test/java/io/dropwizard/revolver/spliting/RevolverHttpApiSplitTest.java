...
public class RevolverHttpApiSplitTest extends BaseRevolverTest {
    ...
    public void testSimpleSplitHttpCommand() throws TimeoutException {
        ...
        stubFor(get(urlEqualTo("/v2/test")).willReturn(
                aResponse().withStatus(200).withHeader("Content-Type", "application/json")));
        stubFor(get(urlEqualTo("/v1/split")).willReturn(
                aResponse().withStatus(200).withHeader("Content-Type", "application/json")));

        RevolverHttpCommand httpCommand = RevolverBundle.getHttpCommand("test", "test_split");
        val request = RevolverHttpRequest.builder().service("test").api("test_split")
                .method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/split").build();
    ...
    public void testFalseSplitHttpCommand() throws TimeoutException {
        ...
        List<SplitConfig> falseSplitConfigs = Lists
                .newArrayList(splitConfigv1, splitConfigv2, splitConfigv3);
        RevolverConfig.builder().mailBox(InMemoryMailBoxConfig.builder().build())
                ...
                .global(new RuntimeConfig()).service(
                RevolverHttpServiceConfig.builder().authEnabled(false).connectionPoolSize(1)
                        .secured(false).service("test").type("http")
                        .api(RevolverHttpApiConfig.configBuilder().api("test_false_split")
                                ...
                                .path("{version}/test_false_split").splitConfig(
                                        RevolverHttpApiSplitConfig.builder().enabled(true)
                                                .splits(falseSplitConfigs).build()).runtime(
        ...
        val request = RevolverHttpRequest.builder().service("test").api("test_false_split")
                .method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/test_false_split")
                .build();
    ...
    public void testSimpleSplitServiceHttpCommand() throws TimeoutException {
        stubFor(get(urlEqualTo("/v1/test_service_split")).willReturn(
                aResponse().withStatus(200).withHeader("Content-Type", "application/json")));

        RevolverHttpCommand httpCommand = RevolverBundle
                .getHttpCommand("test", "test_service_split");
        val request = RevolverHttpRequest.builder().service("test").api("test_service_split")
                .method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/test_service_split")
                .build();
        val response = httpCommand.execute(request);
        assertEquals(200, response.getStatusCode());
   ...
   /* @Test
    ...
    public void testFalsePathExpressionSplitServiceHttpCommand() throws TimeoutException {
        ...
        RevolverHttpCommand httpCommand = RevolverBundle
                .getHttpCommand("test", "test_path_expression");
        val request = RevolverHttpRequest.builder().service("test").api("test_path_expression")
                .method(RevolverHttpApiConfig.RequestMethod.GET).path("v1/test_path_expression")
                .build();
        val response = httpCommand.execute(request);
